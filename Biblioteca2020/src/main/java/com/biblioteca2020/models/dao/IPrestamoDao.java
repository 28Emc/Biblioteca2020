...
public interface IPrestamoDao extends CrudRepository<Prestamo, Long> {
	...
	// USADO
	@Query("select p from Prestamo p join fetch p.usuario pe join fetch p.libro li join fetch p.empleado em where li.id like ?1")
	...
	// USADO
	@Query("select p from Prestamo p join fetch p.usuario pe join fetch p.libro li join fetch p.empleado em where li.id like ?1 and li.local.id like ?2")
